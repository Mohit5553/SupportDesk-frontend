...
import {
    ...
    User, CheckCircle, Upload, MoreHorizontal, Star
} from 'lucide-react';
...
const TicketDetail = () => {
    ...
    const submitFeedbackMutation = useMutation({
        mutationFn: async (data) => {
            const res = await API.post(`/tickets/${id}/feedback`, data);
            return res.data;
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['ticket', id]);
    ...
    const onSubmitFeedback = (e) => {
        ...
        submitFeedbackMutation.mutate({ rating, comment: feedbackComment });
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {['Resolved', 'Closed'].includes(ticket.status) && ticket.createdBy._id === user._id && (
                            <div className={`glass-card p-5 mt-6 border-${ticket.feedback?.rating ? 'emerald' : 'primary'}-500/30 bg-white/5`}>
                                {ticket.feedback?.rating ? (
                                    <>
                                        ...
                                        <div className="flex items-center gap-1 mb-2">
                                            {[1, 2, 3, 4, 5].map((star) => (
                                                <Star key={star} className={`w-5 h-5 ${star <= ticket.feedback.rating ? 'text-yellow-400 fill-yellow-400' : 'text-slate-600'}`} />
                                            ))}
                                        </div>
                                        {ticket.feedback.comment && <p className="text-sm text-slate-300 mt-2">{ticket.feedback.comment}</p>}
                                    </>
                                ) : (
                                    <form onSubmit={onSubmitFeedback} className="space-y-4">
                                        <h3 className="font-semibold text-white">How would you rate your support experience?</h3>
                                        <div className="flex items-center gap-2">
                                            {[1, 2, 3, 4, 5].map((star) => (
                                                ...
                                                >
                                                    <Star className={`w-8 h-8 ${star <= rating ? 'text-yellow-400 fill-yellow-400' : 'text-slate-600 hover:text-yellow-400/50'}`} />
                                                </button>
                                            ))}
                                        </div>
                                        <div>
                                            <textarea
                                                className="form-input bg-dark-900 text-sm w-full min-h-[80px]"
                ...
                <div className="space-y-6">
                    ...
                    {ticket.events && ticket.events.length > 0 && (
                        <div className="glass-card p-5">
                            <h3 className="font-semibold text-white mb-4 border-b border-white/10 pb-3 flex items-center gap-2">
                            ...
                            <div className="space-y-4 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
                                {ticket.events.map((event, idx) => (
                                    <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                                        <div className="flex items-center justify-center w-4 h-4 rounded-full border border-white/10 bg-dark-900 group-[.is-active]:bg-primary-500 text-slate-500 group-[.is-active]:text-emerald-50 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10"></div>
                                        <div className="w-[calc(100%-2rem)] md:w-[calc(50%-1rem)] bg-white/5 border border-white/10 p-3 rounded-lg">
                                            <div className="flex items-center justify-between mb-1">
                                                <span className="font-semibold text-slate-200 text-xs">{event.type}</span>
                                                <span className="text-[10px] text-slate-500">{formatDate(event.createdAt || ticket.createdAt)}</span>
                                            </div>
