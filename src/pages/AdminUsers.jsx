                                            ...
                                            <div className="space-y-2">
                                                ...
                                                {(u.role === 'agent' || u.role === 'manager') && (
                                                    <div className="flex flex-col gap-1">
                                                        <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Level & Shift</span>
                                                        <div className="flex gap-2">
                                                            <span className="text-xs bg-primary-500/10 text-primary-400 px-2 py-0.5 rounded border border-primary-500/20">{u.level || 'L1'}</span>
                                                            <span className="text-xs bg-violet-500/10 text-violet-400 px-2 py-0.5 rounded border border-violet-500/20">{u.shift || 'Day'}</span>
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <td className="p-4">
                                            <div className="flex flex-col gap-3">
                                                ...
                                                {(u.role === 'agent' || u.role === 'manager') && (
                                                    <div className="flex items-center gap-2">
                                                        <select
                                                            className="form-input bg-dark-950 border-white/5 py-1 px-2 text-[11px] w-24"
                                                            value={u.level || 'Level 1'}
                                                            onChange={(e) => updateUserMutation.mutate({ id: u._id, updates: { level: e.target.value } })}
                                                        >
                                                        ...
                                                        <select
                                                            className="form-input bg-dark-950 border-white/5 py-1 px-2 text-[11px] w-24"
                                                            value={u.shift || 'Day'}
                                                            onChange={(e) => updateUserMutation.mutate({ id: u._id, updates: { shift: e.target.value } })}
                                                        >
